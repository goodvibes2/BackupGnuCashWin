...
The BackupGnuCash project is not part of the GnuCash project.
...
GNU/Linux:
  ...
  Java:
    ...
    Open Java
      ...
      If openjfx is not available from your distributions repositories, try
...
```
  Ubuntu
  To set up a BackupGnuCash.desktop file so it can be started from the Unity
  Dash:
...
```
  (Ensure the Exec= line points to where you put BackupGnuCash.jar)
...
```
  ...
  2) Open the required Linux or Windows URL from above in a web browser,
     ...
     I suggest after extracting, renaming folder
...
can, so long as Java 8 (open or Oracle), JavaFX or openJFX, and 7-Zip are
available.
